...
export const Users: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (token && profile) {
      setLoading(true);
      getPromise<{ data: { users: User[]; count: number } }>(
        `admin/user?isCustomer=${window.location.href.includes(
          "customers"
        )}&rowsPerPage=${rpg}&page=${page}&filter=${filter}`,
        token
      ).then((res) => {
        if (fetchCanceled) {
          return;
  ...
  function getUsers(token: string, filter: string, callBack?: Function) {
    setLoading(true);
    getPromise<{ data: { users: User[]; count: number } }>(
      `admin/user?isCustomer=${window.location.href.includes(
        "customers"
  ...
  const DeleteComponent: React.FC<{ user: User }> = ({ user }) => {
    ...
    return (
      <div className="flex flex-col">
        ...
        <div className="my-5">
          Are you sure you want to delete user with nina: <br /> {user.nina}{" "}
          <br />
        ...
        <div className="flex flex-row justify-between w-full">
          <Button
            ...
            onClick={() => {
              setLoading(true);
              deleteRequest(`/admin/user/${user.id}`, token || "")
                .then((res) => {
                  getUsers(token || "", filter, () => closeDialog());
                })
                .catch((error) => {
                  setLoading(false);
      ...
      <div className="no-shadow flex flex-col bg-main bottom-height justify-between style-1 overflow-auto">
        ...
        >
          <div className="border border-green rounded-lg rounded-br-none flex flex-col">
            <div
              className="grid grid-cols-8 bg-main justify-between  border-b border-green  rounded-t-lg green  py-2 font-16"
              style={{ fontSize: 16 }}
            >
              <span className="col-span-2 text-left ml-4 flex flex-row">
                {" "}
                <div
            ...
            <div className=" rounded-lg ">
              {loading ? (
                ...
                rows.map((row, indx) => (
                  <div
                    className={`grid grid-cols-8 bg-main justify-between rounded-lg font-size pb-5 green mt-4`}
                    style={{ fontSize: 14 }}
                    key={row.id}
                  >
                    {" "}
