...
import { Warehouse } from "../warehouse/warehouses";
...
export const Vendors: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (token && profile) {
      setLoading(true);
      getPromise<{ data: { vendors: Vendor[]; count: number } }>(
        `admin/vendors?rowsPerPage=${rpg}&page=${page}&filter=${filter}`,
        token
      ).then((res) => {
        if (fetchCanceled) {
          return;
  ...
  function getVendors(token: string, filter: string, callBack?: Function) {
    setLoading(true);
    getPromise<{ data: { vendors: Vendor[]; count: number } }>(
      `admin/vendors?rowsPerPage=${rpg}&page=${page}&filter=${filter}`,
      token
  ...
  const DeleteComponent: React.FC<{ vendor: Vendor }> = ({ vendor }) => {
    ...
    return (
      <div className="flex flex-col">
        ...
        <div className="my-5">
          {t("DELETE_SURE_VENDOR")}: <br /> {vendor.name} <br />
        ...
        <div className="flex flex-row justify-between w-full">
          <Button
            ...
            onClick={() => {
              setLoading(true);
              deleteRequest(`/admin/vendros/${vendor.id}`, token || "")
                .then((res) => {
                  getVendors(token || "", filter, () => closeDialog());
                })
                .catch((error) => {
                  setLoading(false);
    ...
    <div className="relative" style={{ marginTop: 78 }}>
      ...
      >
        <div className="flex flex-row justify-between green w-full ">
          <div>
            <span className="font-bold ">{t('MANAGE')} {t('VENDORS')} &nbsp;</span> / &nbsp;
            <span className="text-white">{t('VENDORS')}</span>
          </div>
              ...
              <span className="absolute top-10 bg-white rounded-b-md w-20 h-20 left-4 flex flex-col ">
                <span
                  ...
                  onClick={() => {
                    ...
                    showDialog(
                      <div className="green ">
                        {t("ACTIVATE_VENDOR_SURE")}?
                        <div className="flex flex-row justify-center mt-4 ">
                          <Button
                            onClick={() => {
                              put(
                                "admin/vendors/status/batch",
                                { ids: selected, status: "active" },
                                token ?? ""
                              )
                                .then((res) => {
                                  getVendors(token ?? "", filter, () => {
                                    closeDialog();
                                    setSelected([]);
                                  });
                                })
